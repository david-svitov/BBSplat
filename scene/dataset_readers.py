...
def fetchPly(path, max_points=60_000):
    ...
    if len(positions) >= max_points:
        #indices = np.random.randint(0, len(positions), size=max_points
        _, indices = sample_farthest_points(torch.tensor(positions[None]), K=max_points)
        indices = indices[0]
...
def readColmapSceneInfo(path, images, eval, llffhold=8, max_points=60_000):
    try:
    ...
    try:
        pcd = fetchPly(ply_path, max_points)
    except:
        pcd = None
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png", max_points=60_000):
    ...
    for i, (index, camera_info) in enumerate(camera_info_list.items()):
        ...
        if camera_info['flg'] == 2:
            ...
            if int(index) in valid_list:
                ...
                alpha = Image.fromarray(np.array(np.tile(img_mask[..., np.newaxis], (1, 1, 3)) * 255.0, dtype=np.byte),
                                        "RGB")
